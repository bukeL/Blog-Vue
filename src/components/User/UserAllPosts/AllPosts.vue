...
<script>
  ...
  methods:{
    ...
    searchByIdAndStatus(category,status,page,size){
      ...
      this.$axios.get('userSpecialpost',{params:{category:category,status:status,page:page,size:size}}) 
      .then(res => {
        if(res.data.code == -1){
          // alert('请先登录')
          this.$router.push({name:'UserLogin'})
    ...
    getPostAgain(size){
      this.isShowLoad = true
      this.$axios.get('userSpecialpost',{params:{category:this.myCatgory,status:this.myStatus,page:1,size:size}}) 
      .then(res => {
        if(res.data.code == -1){
          // alert('请先登录')
          this.$router.push({name:'UserLogin'})
